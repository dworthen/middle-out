...
export function registerMetaData<T>(
  name: string
): (config: T) => (target: { [key: string]: any }, property: string) => void {
  ...
  return config => {
    if (config === undefined || config === null) {
      throw new TypeError(
        `Invalid argument 'config'. Expecting non-primative type but received ${config}.`
      );
    }
